...
> 本地 ETF 日频回测能力。同花顺已支持连接管理，采集脚本尚未接入；A 股股票、期货，
...
当前同花顺尚无采集脚本，不能作为本地回测行情来源。鉴权后的
`GET /api/data-sources/tonghuashun/interfaces` 提供 94 个文档接口的目录，
其中验证状态和采集状态分别为尚未验证、尚未实现；该目录不会发起采集。
...
| 方向 | 当前状态 | 后续规划 |
| --- | --- | --- |
| 数据源 | Tushare ETF 与日历采集；同花顺连接管理、接口目录和通用客户端 | 分阶段接入同花顺采集脚本，扩展多源数据覆盖 |

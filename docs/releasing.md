...
自托管脚本从同一 tag 的源码同时构建前端和后端。两个构建都会读取根目录 `VERSION`：前端在
侧栏展示它，后端通过 `GET /api/system/version` 返回它。登录后管理台会拒绝与自身版本不同的
后端，避免混用不同正式版本。

后续引入镜像仓库时，Release 工作流必须从该 tag 的单个提交构建两份镜像，发布它们的不可变
digest，并让部署清单同时引用这两个 digest。正式环境不得混用不同 tag 构建的前端和后端。

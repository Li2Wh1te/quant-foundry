...
export function ReportAssetsPage(){
  ...
  async function compare(r:ReleaseItem,cursor?:string,comparisonPage=1){
    ...
    try{const changed=await foundationApi<{items:{target_key:string;period_start:string;period_end:string;kind:string}[];next_cursor:string|null}>(`/datasets/${ID}/release-changes`,c.signal,{...request(),selected_report:undefined,previous_release:r.parent_id,current_release:r.id,page_size:100,...(cursor?{cursor}:{})});
      if(!c.signal.aborted)setPanel({title:'整份报告版本变化',wide:true,content:<><p>比较固定父版本与所选版本。成员按整份报告替换；受当前问题限制的内容不参与比较。</p>{changed.items.map(i=><p key={i.target_key}>{i.period_start} 至 {i.period_end} · {({added:'新增报告',whole_object_changed:'整份报告内容或成员依据变化',basis_changed:'治理依据变化',inherited:'沿用原报告',withdrawn:'已撤回',blocked:'已阻断',gap:'缺口',restricted_comparison:'当前限制，无法比较',absent:'对象不在新版本'} as Record<string,string>)[i.kind]||'证据不足'}</p>)}<p>第 {comparisonPage} 页</p>{comparisonPage>1&&<button className="qfo-secondary-btn" onClick={()=>compare(r)}>返回第一页</button>}{changed.next_cursor&&<button className="qfo-secondary-btn" onClick={()=>compare(r,changed.next_cursor!,comparisonPage+1)}>下一页变化</button>}</>});
    }catch(e){if(!c.signal.aborted)setError(failure(e));}
  }

...
const PARAMETER_COPY: Record<string, [string, string]> = {
  ...
  native_dataset: ["本地数据领域", "填写已具备正式化适配器的来源领域键；只处理已落地数据，不请求供应商。"],
  backfill_campaign_id: ["全量捕获周期", "填写固定全量捕获的周期 ID；该领域全量正式发布完成后才开始持续更新。"],
  execution_id: ["正式化执行版本", "填写已登记并完成运行时归档验证的执行版本 ID。"],
  runtime_digest: ["运行时镜像摘要", "填写该执行版本归档的 sha256 镜像摘要；不匹配时停止处理。"],
  source_limit: ["每批来源版本数", "每次最多轮转这些本地版本，历史与当前版本分别保留处理机会。"],
  steps_per_source: ["每个来源的处理单元数", "本次预算耗尽后保存工作进度，下次继续；单元完成不代表全量完成。"],

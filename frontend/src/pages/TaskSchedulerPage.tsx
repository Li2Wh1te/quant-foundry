...
import { canRun, formatSchedule, nextRunLabel, runStateLabel, runSummary, taskStateLabel, taskTime } from "./tasks/taskPresentation";
      ...
      <div className={`qft-workspace-body${selectedTask ? " qft-with-detail" : ""}`}>
        <div className="qft-table-column"><div className="qft-table-wrap" tabIndex={0} role="region" aria-label="采集任务表格，可横向滚动" aria-busy={loading}>
          <table className="qft-table"><colgroup><col style={{ width: "28%" }} /><col style={{ width: "11%" }} /><col style={{ width: "17%" }} /><col style={{ width: "10%" }} /><col style={{ width: "14%" }} /><col style={{ width: "14%" }} /><col style={{ width: "74px" }} /></colgroup><thead><tr><th scope="col">任务</th><th scope="col">数据源</th><th scope="col">执行计划</th><th scope="col">状态</th><th scope="col">最近运行</th><th scope="col">下次执行</th><th scope="col"><span className="qft-sr-only">操作</span></th></tr></thead><tbody>
            {data?.items.map(task => <tr key={task.id} className={task.id === selected ? "qft-selected" : ""} onClick={() => setSelected(task.id)}><td><button className="qft-task-name" type="button" aria-expanded={selected === task.id} onClick={() => setSelected(task.id)}>{task.name}</button><small>{task.task_type_name ?? "未注册任务类型"}{task.task_type_english_name && `（${task.task_type_english_name}）`}</small></td><td><strong>{sourceName(task)}</strong>{task.source_enabled === false && <small>已停用</small>}</td><td>{formatSchedule(task.schedule)}<small>{task.schedule.type === "cron" ? task.schedule.timezone : "上海时间"}</small></td><td><span className={`qft-state qft-state-${task.state}`}>{taskStateLabel(task.state)}</span>{(task.running_count > 0 || task.queued_count > 0) && <small>运行 {task.running_count} · 等待 {task.queued_count}</small>}</td><td>{task.active_run ? <>{runStateLabel(task.active_run.status)}<small>{runSummary(task.active_run)}</small></> : task.latest_run ? <>{taskTime(task.latest_run.started_at ?? task.latest_run.created_at)}<small>{runStateLabel(task.latest_run.status)}</small></> : "暂无运行"}</td><td>{nextRunLabel(task)}</td><td><button className="qft-mini-btn" type="button" disabled={!canRun(task)} aria-disabled={busy || loading || !!error} title={canRun(task) ? "触发一次运行" : nextRunLabel(task)} onClick={event => { event.stopPropagation(); void act(task, "run"); }}>运行</button></td></tr>)}
          </tbody></table>

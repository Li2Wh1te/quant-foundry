...
test('current local update has Chinese outcomes without leaking raw errors',()=>{
  assert.match(view.runSummary({task_type:'data_store.update_local',status:'failed',error_type:'RuntimeError',error_message:'上游原文不应显示'}),/本地数据更新失败/);
  assert.doesNotMatch(view.runSummary({task_type:'data_store.update_local',status:'failed',error_type:'VendorError',error_message:'上游原文不应显示'}),/上游原文/);
  assert.match(view.runSummary({task_type:'data_store.update_local',status:'queued'}),/已排队/);
  // Historical logs still render their recorded Chinese title after retirement.

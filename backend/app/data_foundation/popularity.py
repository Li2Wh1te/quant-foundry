...
def popularity_body(source, raw):
    ...
    if not isinstance(raw.get('item'), list):
        raise FoundationError('SOURCE_SCHEMA_INVALID', '排行观察缺少明确的完整成员列表。')
    ...
    if scope is not None and (not isinstance(scope, dict) or scope.get(expected_scope[0]) != expected_scope[1]):
        raise FoundationError('IDENTITY_CONFLICT', '排行采集范围与固定来源主体不一致。')
    members = []
    for row in raw['item']:
        if not isinstance(row, dict):
            raise FoundationError('SOURCE_SCHEMA_INVALID', '排行存在非对象成员，整组隔离。')
        code = row.get('thscode')
        if not isinstance(code, str) or not code.strip() or '.' not in code:
            raise FoundationError('IDENTITY_UNRESOLVED', '排行成员缺少来源代码，整组隔离。')
        ...
        for original, target in [('name', 'name'), ('ticker', 'ticker'), ('rank_trend', 'reported_rank_trend')]:
            ...
            if value is not None and not isinstance(value, str):
                raise FoundationError('SOURCE_SCHEMA_INVALID', '排行成员报告文本类型无效，整组隔离。')
            member[target] = value
        ...
        if type(member['reported_rank']) is not int or member['reported_rank'] < 1:
            raise FoundationError('CORE_VALUE_INVALID', '排行成员缺少有效的来源名次，未按数组位置补造。')

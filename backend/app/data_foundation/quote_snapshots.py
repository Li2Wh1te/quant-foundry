...
def snapshot_body(source, raw):
    ...
    if not isinstance(code, str) or not code.strip() or '.' not in code:
        raise FoundationError('IDENTITY_UNRESOLVED', '行情估值观察缺少明确来源标的代码。')
    if not isinstance(rows, list) or len(rows) != 1 or not isinstance(rows[0], dict):
        raise FoundationError('SOURCE_SCHEMA_INVALID', '行情估值观察须恰有一个明确主体记录，空项或重复项整组隔离。')
    row = rows[0]
    if row.get('thscode') != code:
        raise FoundationError('IDENTITY_CONFLICT', '行情估值记录与固定来源主体不一致。')
    ...
    if scope is not None and (not isinstance(scope, dict) or any(
        ...
        raise FoundationError('IDENTITY_CONFLICT', '行情估值采集范围与固定来源主体不一致。')
    ...
    if stamp is not None:
        try:
            ...
            raise FoundationError('SOURCE_SCHEMA_INVALID', '行情估值来源报告时间不是有效的整数毫秒时间。') from None
    ...
    for native, target in [('ticker', 'reported_ticker'), ('name', 'reported_name')]:
        ...
        if value is not None and (not isinstance(value, str) or not value.strip()):
            raise FoundationError('SOURCE_SCHEMA_INVALID', '行情估值名称或展示代码不是有效文本。')
        body[target] = value
        if value is None:
    ...
    try:
        ...
        raise FoundationError('CORE_VALUE_INVALID', '行情估值报告量值无效，未将非数值或精度溢出转换为零。') from None
    if any(body['reported_' + field] is not None for field in fields):
        ...
        if not all(field in row and row[field] is None for field in expected):
            raise FoundationError('CORE_VALUE_INVALID', '行情估值量值字段缺失，无法确认为来源明确返回的空值观察。')

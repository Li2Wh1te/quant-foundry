...
def nav_body(source, raw):
    ...
    from app.data_foundation.record_adapters import daily_decimal, source_date
    key = source.subject
    if not isinstance(key, str) or not key.strip():
        raise FoundationError('IDENTITY_UNRESOLVED', '基金净值窗口缺少明确的固定来源主体。')
    rows = raw.get('points')
    if raw.get('coverage') != 'provider_rolling_window' or not isinstance(rows, list) or not rows:
        raise FoundationError('SOURCE_SCHEMA_INVALID', '基金净值缺少明确的滚动窗口或非空完整序列，整组隔离。')
    points = []
    try:
        for row in rows:
            ...
            if row.get('thscode') not in (None, key):
                raise FoundationError('IDENTITY_CONFLICT', '基金净值成员主体与固定来源主体不一致，整组隔离。')
        ...
        raise FoundationError('CORE_VALUE_INVALID', '基金净值存在无效日期、重复日期或非精确数值，整组隔离且不拼接旧序列。') from None

...
def experience_body(source, raw):
    ...
    if not isinstance(manager, str) or not manager.strip():
        raise NativeInputError('IDENTITY_UNRESOLVED', '经理经历缺少固定来源经理标识。')
    if not isinstance(history, dict):
        raise NativeInputError('SOURCE_SCHEMA_INVALID', '经理经历缺少明确的任职集合，未按空集合发布。')
    assignments, quality = [], {}
    for code, entry in sorted(history.items()):
        if (not isinstance(code, str) or not code.strip() or not isinstance(entry, dict)
            ...
            raise NativeInputError('IDENTITY_CONFLICT', '经理任职基金代码与集合键不一致，保留整组待修复。')
        item = {'fund_code': code}
        for original, target in (('name', 'name'), ('hqcode', 'quotation_code'), ('type', 'fund_type'),
            ...
            if value is not None and not isinstance(value, str):
                raise NativeInputError('SOURCE_SCHEMA_INVALID', '经理任职文本类型无效，未将异常成员当作退出。')
            item[target] = value

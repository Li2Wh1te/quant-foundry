...
def daily_window_body(source, raw):
    ...
    if not isinstance(code, str) or not code.strip() or '.' not in code:
        raise NativeInputError('IDENTITY_UNRESOLVED', '日线窗口缺少明确的来源标的代码。')
    asset, adjustment = BASES[source.dataset]
    rows = raw.get('item')
    if raw.get('adjust') != adjustment or raw.get('coverage') != 'observed_rows_only':
        raise NativeInputError('SOURCE_SCHEMA_INVALID', '日线调整口径或观察范围不符合已验证来源契约。')
    if not isinstance(rows, list) or not rows:
        raise NativeInputError('SOURCE_SCHEMA_INVALID', '日线窗口缺少明确的非空完整序列，未补造零值行情。')
    if raw.get('thscode') not in (None, code):
        raise NativeInputError('IDENTITY_CONFLICT', '日线容器标的与固定来源不一致。')
    envelopes = raw.get('provider_envelopes', [])
    if not isinstance(envelopes, list):
        raise NativeInputError('SOURCE_SCHEMA_INVALID', '日线来源响应容器类型无效。')
    for envelope in envelopes:
        if not isinstance(envelope, dict) or envelope.get('thscode') not in (None, code) or envelope.get('interval') not in (None, '1d'):
            raise NativeInputError('IDENTITY_CONFLICT', '日线响应标的或频率与固定来源不一致。')
        if envelope.get('adjust') not in (None, adjustment):
            raise NativeInputError('SOURCE_SCHEMA_INVALID', '日线响应存在冲突调整口径，未拼接不同复权基准。')
    points = []
    try:
        for row in rows:
            ...
            if row.get('thscode') not in (None, code):
                raise NativeInputError('IDENTITY_CONFLICT', '日线成员标的与固定来源不一致，整组隔离。')
            if row.get('interval') not in (None, '1d') or any(row.get(field) not in (None, adjustment) for field in ('adjust', 'adjusted')):
                raise NativeInputError('SOURCE_SCHEMA_INVALID', '日线成员频率或调整口径冲突，整组隔离。')
        ...
        raise NativeInputError('CORE_VALUE_INVALID', '日线窗口存在无效日期、重复日期或非精确数值，整组隔离。') from None

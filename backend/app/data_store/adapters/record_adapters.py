...
from app.data_store.adapters.canonical import NativeInputError, normalized
from app.data_store.adapters.record_schemas import validate_body, schema_for
...
def dataset_for(source):
    try:
        ...
        raise NativeInputError('DOMAIN_NOT_IMPLEMENTED', '该来源的类型化领域适配器尚未安装。') from None
...
def rows_for(source, content):
    if source.representation == 'local_table_baseline':
        ...
        if not isinstance(content, dict):
            raise NativeInputError('SOURCE_SCHEMA_INVALID', '固定来源缺少完整对象容器。')
        rows = content.get('item')
    if not isinstance(rows, list):
        raise NativeInputError('SOURCE_SCHEMA_INVALID', '固定来源缺少明确的记录列表，未将缺项当作空结果。')
    ...
    if source.source == 'tonghuashun' and source.dataset == 'fund_manager_experience' and len(rows) != 1:
        raise NativeInputError('SOURCE_SCHEMA_INVALID', '经理经历须有一个明确的任职集合容器，未猜测缺失或重复容器。')
    if source.source == 'tonghuashun' and source.dataset == 'fund_nav':
        ...
        if not isinstance(content, dict):
            raise NativeInputError('SOURCE_SCHEMA_INVALID', '基金净值必须保留完整来源窗口容器。')
        if content.get('thscode') not in (None, source.subject):
            raise NativeInputError('IDENTITY_CONFLICT', '基金净值容器主体与固定来源主体不一致。')
    ...
    if source.source == 'tonghuashun' and source.dataset == 'fund_offerings':
        if not isinstance(content, dict):
            raise NativeInputError('SOURCE_SCHEMA_INVALID', '募集列表须保留完整来源容器。')
        ...
        if scope is not None and (not isinstance(scope, dict) or scope.get('subscribe') != source.subject):
            raise NativeInputError('IDENTITY_CONFLICT', '募集列表筛选分类与固定来源主体不一致。')
        return [{'members': rows}]
...
def convert(source, raw):
    ...
    if not isinstance(raw, dict):
        raise NativeInputError('SOURCE_SCHEMA_INVALID', '来源记录不是字段对象。')
    quality = {}
    def text(name, target=None, required=False):
        ...
        if isinstance(value, str) and value.strip():
            return value
        if required:
            raise NativeInputError('IDENTITY_UNRESOLVED', '记录缺少明确的来源主体标识。')
    ...
    if source.source == 'tonghuashun' and source.dataset in ('fund_news', 'anomaly_stock', 'rank_trend'):
        from app.data_store.adapters.narrative_windows import narrative_body
        ...
        from app.data_store.adapters.portfolio_windows import portfolio_body
        ...
        from app.data_store.adapters.stock_indicators import indicators_body
        ...
        from app.data_store.adapters.fund_ownership import ownership_body
        body, quality = ownership_body(source, raw)
        key, kind = source.subject, 'asset:fund'
    elif source.source == 'tonghuashun' and source.dataset in ('fund_dividends', 'stock_actions'):
        from app.data_store.adapters.distributions import distribution_body
        ...
        from app.data_store.adapters.manager_performance import performance_body
        ...
        from app.data_store.adapters.manager_style import style_body
        body, quality = style_body(source, raw)
        key, kind = source.subject, 'fund_manager'
    elif source.source == 'tonghuashun' and source.dataset in ('fund_returns','fund_drawdowns','fund_performance_history'):
        from app.data_store.adapters.fund_performance import performance_body
        body, quality = performance_body(source, raw)
        key, kind = source.subject, 'asset:fund'
    elif source.source == 'tonghuashun' and source.dataset == 'dragon_tiger':
        from app.data_store.adapters.dragon_tiger import dragon_tiger_body
        body, quality = dragon_tiger_body(source, raw)
        key, kind = 'dragon_tiger:' + source.subject, 'dragon_tiger_list'
    elif source.source == 'tonghuashun' and source.dataset in ('stock_auction', 'auction_benchmark', 'limit_up', 'limit_down', 'limit_break', 'anomaly_list', 'limit_ladder'):
        from app.data_store.adapters.market_activity import activity_body
        ...
        from app.data_store.adapters.quote_snapshots import snapshot_body
        ...
        from app.data_store.adapters.popularity import popularity_body
        ...
        from app.data_store.adapters.fund_quotas import quota_body
        ...
        for member in members:
            ...
            for field in ('name', 'ticker'):
                ...
                if value is not None and (not isinstance(value, str) or not value.strip()):
                    raise NativeInputError('SOURCE_SCHEMA_INVALID', '指数集合成员文本类型无效，整组隔离。')
        ...
        if tushare:
            ...
            raise NativeInputError('IDENTITY_CONFLICT', '目录资产分类与固定来源范围不一致。')
        ...
        if raw.get('source') != 'tushare':
            raise NativeInputError('IDENTITY_CONFLICT', '日线记录来源与固定来源命名空间不一致。')
        ...
        for field in ('open', 'high', 'low', 'close'):
            try:
                ...
                raise NativeInputError('CORE_VALUE_INVALID', '日线核心价格缺失、无效或超出精度，未补造价格。') from None
        ...
        if isinstance(factor, bool) or not isinstance(factor, (str, int, Decimal)):
            raise NativeInputError('CORE_VALUE_INVALID', '复权因子缺少精确十进制表示。')
        try:
            number = Decimal(factor)
        except InvalidOperation:
            raise NativeInputError('CORE_VALUE_INVALID', '复权因子不是有效十进制值。') from None
        if not number.is_finite() or number <= 0:
            raise NativeInputError('CORE_VALUE_INVALID', '复权因子必须为有限正数。')
        body = dict(source_code=key, trade_date=source_date(raw.get('trade_date')),
        ...
        if source.source == 'tushare':
            ...
            if not isinstance(opened, bool):
                raise NativeInputError('CORE_VALUE_INVALID', '交易日期缺少明确的布尔开闭市状态。')
            ...
            if raw.get('date_ms') is not None and raw.get('date') is not None and source_date(raw['date']) != business_date:
                raise NativeInputError('CORE_VALUE_INVALID', '交易日期两种编码不一致，未选择其中一种覆盖冲突。')
    ...
    if source.source == 'tonghuashun' and source.dataset in ('fund_profile', 'fund_company', 'fund_manager') and key != source.subject:
        raise NativeInputError('IDENTITY_CONFLICT', '来源容器主体与记录主体不一致，未跨主体发布。')

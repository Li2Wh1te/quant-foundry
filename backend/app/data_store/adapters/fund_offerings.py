...
def offering_body(source, raw):
    ...
    if key not in ('active', 'upcoming'):
        raise NativeInputError('IDENTITY_UNRESOLVED', '募集列表缺少已验证的来源筛选分类。')
    if not isinstance(rows, list):
        raise NativeInputError('SOURCE_SCHEMA_INVALID', '募集列表缺少明确的完整成员集合。')
    members = []
    try:
        for row in rows:
        ...
        raise NativeInputError('CORE_VALUE_INVALID', '募集列表存在无效主体或时间编码，整组隔离，未丢弃成员或截断日内时间。') from None

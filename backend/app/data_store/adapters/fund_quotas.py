...
def required_text(row, key):
    ...
    if not isinstance(value, str):
        raise NativeInputError('SOURCE_SCHEMA_INVALID', 'QDII报告文本缺失或类型无效，未将缺项改成零。')
    return value
...
def quota_body(source, raw):
    ...
    if not isinstance(rows, list) or len(rows) > 1:
        raise NativeInputError('SOURCE_SCHEMA_INVALID', 'QDII请求分类须返回明确的零或一个分类容器，未合并冲突分类。')
    if not isinstance(source.subject, str) or not source.subject.strip():
        raise NativeInputError('IDENTITY_UNRESOLVED', 'QDII观察缺少固定请求分类。')
    if raw.get('category_scope') not in (None, 'explicit_categories_only'):
        raise NativeInputError('SOURCE_SCHEMA_INVALID', 'QDII观察的分类覆盖范围未经验证。')
    ...
    if scope is not None:
        try:
            ...
            valid = False
        if not valid:
            raise NativeInputError('IDENTITY_CONFLICT', 'QDII请求分类与固定来源主体不一致。')
    groups = []
    summary = source.dataset == 'fund_quota_summary'
    for row in rows:
        if not isinstance(row, dict) or row.get('name') != source.subject:
            raise NativeInputError('IDENTITY_CONFLICT', 'QDII响应包含其他分类，整组隔离。')
        ...
        if summary:
            ...
            if not isinstance(row.get('sub_tab'), list):
                raise NativeInputError('SOURCE_SCHEMA_INVALID', 'QDII分类缺少明确的子分类列表。')
            subcategories = []
            for tab in row['sub_tab']:
                if not isinstance(tab, dict) or not isinstance(tab.get('fund_list'), list):
                    raise NativeInputError('SOURCE_SCHEMA_INVALID', 'QDII子分类缺少明确的基金列表，整组隔离。')
                funds = []
                for fund in tab['fund_list']:
                    if not isinstance(fund, dict):
                        raise NativeInputError('SOURCE_SCHEMA_INVALID', 'QDII基金成员不是有效对象，整组隔离。')
                    code = required_text(fund, 'thscode')
                    if '.' not in code or not code.strip():
                        raise NativeInputError('IDENTITY_UNRESOLVED', 'QDII基金成员缺少来源代码。')
                    ...
                    if classify is not None and (not isinstance(classify, list) or any(not isinstance(v, str) for v in classify)):
                        raise NativeInputError('SOURCE_SCHEMA_INVALID', 'QDII基金分类标签类型无效，整组隔离。')
